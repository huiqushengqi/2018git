...
public class UserDao {
	...
	public int addUser(User user, Connection conn){
		String sql = "INSERT INTO user(user_id,user_name,user_pass,real_name,user_sex,user_age,user_nation,user_origin,user_mail,user_phone,user_photo,clazz_id,reg_date,user_type,user_question,user_answer,note) values(null,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int updateUser(User user, Connection conn){
		...
		if (!StringUtil.isEmptyString(user.getUser_origin())) {
			sBuilder.append(" ,user_origin ='" + user.getUser_origin() +"' ");
		}
		...
		if (!StringUtil.isEmptyString(user.getUser_photo())) {
			sBuilder.append(" ,user_photo ='" + user.getUser_photo() +"' ");
		}
	...
	public List<User>  listUsers(User user, Connection conn){
		...
		if (user.getCourse_id()!=0) {
			sBuilder.append(" and (u.user_id in( select user_id from scource where course_id=" + user.getCourse_id() +" ");
			sBuilder.append(" 						and scource_year=" + user.getScource_year() +" ");
			sBuilder.append("						and scource_year_half=" + user.getScource_year_half() +")) ");
		}
