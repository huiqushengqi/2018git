...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : 'images/nicEditorIcons.png',
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			var s = {width: newX+'px', overflowY : 'auto', overflowX: 'hidden' };
			s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight+'px' : null;
			this.editorContain = new bkElement('DIV').setStyle(s).appendBefore(e).addClass('container');
			var editorElm = new bkElement('DIV').setStyle({width : (newX-24)+'px', margin: '8px 12px', minHeight : newY+'px'}).addClass('main').appendTo(this.editorContain);
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
	...
	initFrame : function() {
		...
		fd.write('<thymeleaf><head>'+((css) ? '<link href="'+css+'" rel="stylesheet" type="text/css" />' : '')+'</head><body id="nicEditContent" style="margin: 0 !important; background-color: transparent !important;">'+this.initialContent+'</body></thymeleaf>');
		fd.close();
		this.frameDoc = fd;
...
var nicEditorButton = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('DIV').setStyle({width : '30px', height : '26px'}).addClass('buttonContain').appendTo(this.margin);
		this.border = new bkElement('DIV').appendTo(this.contain);
		this.button = new bkElement('DIV').setStyle({width : '18px', height : '20px', overflow : 'hidden', zoom : 1, cursor : 'pointer'}).addClass('button').setStyle(this.ne.getIcon(buttonName,options)).appendTo(this.border);
	...
	setBg : function(state) {
		switch(state) {
			case 'hover':
				var stateStyle = {backgroundColor : '#dad1bf'};
				break;
			case 'active':
				var stateStyle = {backgroundColor : '#dad1bf'};
				break;
			default:
				var stateStyle = {backgroundColor : 'transparent', borderRight : '1px solid #adadad', padding : '4px 0 2px'};
		}
